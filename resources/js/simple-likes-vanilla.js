/**
 * Simple Likes - Vanilla JavaScript
 *
 ...
 *
 * Features:
 * - Static cache compatible (fetches status on init)
...
(function() {
    ...
    function initComponent(container) {
        ...
        var state = {
            ...
            liked: false, // Will be hydrated from API
        ...
        function updateDisplay() {
            ...
            if (heartEl) {
                ...
                // Update colour based on hydration and liked state
                if (!state.hydrated) {
                    heartEl.style.color = '#9ca3af'; // Grey while loading
                } else {
                    heartEl.style.color = '#ef4444'; // Red when hydrated
                }
            }
            if (button) {
            ...
            // Update container opacity during loading
            container.style.opacity = state.hydrated ? '1' : '0.5';
        }

        /**
         * Fetch real like status from server (for static cache compatibility)
         */
        function hydrateStatus() {
            ...
            fetch('/!/simple-likes/status?ids=' + state.entryId, {
                method: 'GET',
                credentials: 'same-origin'
            })
            .then(function(response) {
            ...
            .then(function(data) {
                if (data[state.entryId]) {
                    state.liked = data[state.entryId].liked;
                    state.count = data[state.entryId].count;
                }
            })
            .catch(function(e) {
                // Silently fail - use server-rendered values as fallback
                console.warn('Simple Likes: Failed to hydrate status', e);
            })
            .finally(function() {
                state.loading = false;
                state.hydrated = true;
        ...
        // Initial display update (shows loading state)
        updateDisplay();

        // Hydrate status from API
        hydrateStatus();
